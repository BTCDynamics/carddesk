# CardDesk Database Map

_Last updated: 2026-06-10_

This document summarizes the SQLAlchemy models currently defined in `models.py`.
...
- Local development commonly uses SQLite.
- Render may use a different database file/location or persistent disk configuration than local development.
- A page can work locally and fail on Render if Render's database schema is older than `models.py`.
- Before a deploy that changes `models.py`, run the DB schema audit locally and plan any Render database migration/update.

## Common Schema Error Patterns

| Error | Meaning |
|---|---|
| `no such table` | The deployed database does not have a model table yet |
| `no such column` | `models.py` expects a column not present in the database |
| `OperationalError` during query | Usually schema mismatch, bad path, or missing database file |
| Works local, fails Render | Render database state does not match local database state |
...
This should be read-only and should not alter data.

## Backup Reminder

Before any database migration or large feature update, back up:
